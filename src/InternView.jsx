    ...
    <div style={{ background:P.bg,minHeight:'100vh',color:P.tx,fontFamily:"'IBM Plex Mono','SF Mono',monospace",fontSize:13 }}>
      ...
      <div style={{ maxWidth:1200,margin:'0 auto',padding:'24px 16px' }}>
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:24 }}>
          <Card style={{ padding:14,borderLeft:`3px solid ${P.r}` }}><Lbl>Late</Lbl><div style={{ fontSize:28,fontWeight:800,color:P.r }}>{late.length}</div><div style={{ fontSize:10,color:P.tm,marginTop:4 }}>{late.length>0?late.map(o=>`${o.client.nm} (${MO[o.month]})`).join(', '):'None late'}</div></Card>
            ...
            <tbody>{payingClients.map((cl,ci)=>{const ytd=cl.st.filter(s=>s==='P').length*cl.rt;const isExp=expanded===ci;const termLabel=cl.termMo?`${cl.termMo}mo`:(cl.tr||'\u2014');return(
              <tr key={cl.id} style={{ verticalAlign:'top' }}>
                <td style={{ padding:'6px 8px',borderBottom:`1px solid ${P.bd}10` }}>
                  ...
                  {isExp&&(<div style={{ marginTop:10,padding:12,background:P.c1,borderRadius:8,border:`1px solid ${P.bd}`,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
                ...
                <td style={{ padding:'6px 8px',color:P.g,borderBottom:`1px solid ${P.bd}10` }}>{fmt(cl.rt)}/mo</td>
                <td style={{ padding:'6px 8px',color:P.tm,fontSize:11,borderBottom:`1px solid ${P.bd}10` }}>{termLabel}</td>
                {MO.map((_,mi)=>{const active=inTerm(cl,mi);const s=active?(cl.st[mi]||'U'):'';return(<td key={mi} style={{ padding:'2px',textAlign:'center',borderBottom:`1px solid ${P.bd}10` }}><div onClick={()=>active&&cyc(ci,mi)} style={sSty(s,active)}>{active?(s||'U'):''}</div></td>);})}
                <td style={{ padding:'6px 8px',textAlign:'right',color:P.g,fontWeight:600,borderBottom:`1px solid ${P.bd}10` }}>{ytd>0?fmt(ytd):'\u2014'}</td>
              </tr>);})}</tbody>
          </table>
        </div>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginTop:20 }}>
          ...
          <Card style={{ padding:12 }}><Lbl>Unpaid</Lbl><div style={{ fontSize:22,fontWeight:800,color:P.a }}>{fmt(d.cl.reduce((s,x)=>s+x.st.filter(v=>v==='U').length*x.rt,0))}</div></Card>
          <Card style={{ padding:12 }}><Lbl>Late</Lbl><div style={{ fontSize:22,fontWeight:800,color:P.r }}>{fmt(d.cl.reduce((s,x)=>s+x.st.filter(v=>v==='L').length*x.rt,0))}</div></Card>
        </div>
      </div>
    </div>
  );
