...
export default function App() {
  ...
  // V2: Client value ranking with enhanced Zoho display
  ...
  const mg = c.bl.filter(b => b > 0).length;
  const fd = c.bl.findIndex(b => b <= 0);
  const tRv = sm(c.rv);
  const pieD = [
    { label: "Zoho Annual", value: sm(d.rv.za), color: P.t },
    { label: "Zoho Monthly", value: sm(d.rv.zm), color: P.b },
    { label: "Infinity Mirror", value: sm(d.rv.im), color: P.g },
    { label: "Marketing", value: sm(d.rv.mk), color: P.p },
    { label: "One-Time", value: sm(d.rv.ot), color: P.a },
  ];
  const devs = c.at.filter(t => t.dp === "Development");
  const aCl = d.cl.filter(x => x.rt > 0).length;

  // V2: Outstanding invoices calculation
  ...
  // V2: Overlay computation for forecast tab
  ...
  // V2: No blue border on current month - only blue background
  ...
  // Legacy helpers for 12-month views (payroll etc)
  const thCm12 = i => ({ ...th, background:i===cm?P.bB:"transparent",color:i===cm?P.b:P.td,fontWeight:i===cm?700:500 });
  const tdCm12 = i => i===cm?P.bB:"transparent";
  ...
  return (
    <div style={{ background:P.bg,minHeight:"100vh",color:P.tx,fontFamily:"'DM Sans', sans-serif",fontSize:13 }}>
          ...
          <div>
            ...
            <Card style={{ padding:12,marginTop:10 }}>
              ...
              <div style={{ display:"flex",alignItems:"baseline",gap:10 }}>
                ...
                {outstanding > 0 && <span style={{ fontSize:11,color:P.tm }}>If collected: <b style={{ color:P.g }}>+{extraRunway} months</b> runway</span>}
              </div>
            </Card>
          </div>
          <Card><Lbl>2026 Revenue ({fmt(tRv)})</Lbl><Pie data={pieD}/></Card>
        </div>
        {/* V2: Rolling 13-month balance strip — no blue border, only bg */}
        ...
        <div style={{ overflowX:"auto",marginBottom:20 }}>
          <table style={{ width:"100%",borderCollapse:"collapse",fontSize:12 }}>
            ...
            <tbody>
              ...
              {/* V2: Baseline vs Overlay balance comparison */}
      ...
      {/* ===================== CLIENTS ===================== */}
      {tab==="clients"&&(<>
        {/* V2: Tier filter toggles */}
        <div style={{ display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center" }}>
          <button onClick={()=>setClFilter(null)} style={{ fontSize:10,color:clFilter===null?P.tx:P.tm,background:clFilter===null?P.c2:"transparent",padding:"4px 10px",borderRadius:4,fontWeight:600,border:`1px solid ${clFilter===null?P.bd:"transparent"}`,cursor:"pointer",fontFamily:"'DM Sans', sans-serif" }}>All ({d.cl.length})</button>
          {Object.entries(TIERS).map(([k,v])=>{const cnt=d.cl.filter(c=>c.tier===k).length;return cnt?<button key={k} onClick={()=>setClFilter(clFilter===k?null:k)} style={{ fontSize:10,color:clFilter===k?v.c:P.tm,background:clFilter===k?`${v.c}20`:"transparent",padding:"4px 10px",borderRadius:4,fontWeight:600,border:`1px solid ${clFilter===k?`${v.c}40`:"transparent"}`,cursor:"pointer",fontFamily:"'DM Sans', sans-serif" }}>{v.l} ({cnt})</button>:null;})}
        ...
        {/* V2: Expandable client rows with integrated payment tracker */}
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%",borderCollapse:"collapse",fontSize:12 }}>
            <thead><tr>
              ...
              <th style={{ ...th,textAlign:"right",width:100,cursor:"pointer" }} onClick={()=>toggleSort("zhAnnual")}>Zoho{sortIcon("zhAnnual")}</th>
            ...
            <tbody>{filteredClients.map((cl)=>{
              ...
              return(<React.Fragment key={cl.id}>
                <tr style={{ cursor:"pointer" }} onClick={()=>setClExpanded(isExp?null:ci)}>
                  ...
                  {/* V2: Rolling 13-month payment tracker inline */}
                ...
                {/* V2: Expanded detail fields */}
                {isExp && <tr key={cl.id+"_exp"}><td colSpan={5+win.length+2} style={{ padding:"0 8px 12px 28px",borderBottom:`1px solid ${P.bd}20` }}>
                  <div style={{ marginTop:8,padding:14,background:P.c2,borderRadius:8,border:`1px solid ${P.bd}`,display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10 }}>
                    ...
                    <div><div style={{ fontSize:9,color:P.td,textTransform:"uppercase",marginBottom:3 }}>Sub Start</div><input type="date" value={cl.subStart||""} onChange={e=>save({...d,cl:d.cl.map((x,i)=>i!==ci?x:{...x,subStart:e.target.value})})} style={{ background:P.c1,border:`1px solid ${P.bd}`,borderRadius:4,color:P.tx,fontSize:12,padding:"6px 8px",width:"100%",boxSizing:"border-box",fontFamily:"'DM Sans', sans-serif" }}/></div>
                    <div><div style={{ fontSize:9,color:P.td,textTransform:"uppercase",marginBottom:3 }}>Pay Due Day</div><input type="number" value={cl.payDay||""} min={1} max={28} onChange={e=>save({...d,cl:d.cl.map((x,i)=>i!==ci?x:{...x,payDay:+e.target.value})})} style={{ background:P.c1,border:`1px solid ${P.bd}`,borderRadius:4,color:P.a,fontSize:12,fontFamily:"'JetBrains Mono', monospace",padding:"6px 8px",width:"100%",boxSizing:"border-box" }}/></div>
                    <div><div style={{ fontSize:9,color:P.td,textTransform:"uppercase",marginBottom:3 }}>Renewal Date</div><input type="date" value={cl.renewal||""} onChange={e=>save({...d,cl:d.cl.map((x,i)=>i!==ci?x:{...x,renewal:e.target.value})})} style={{ background:P.c1,border:`1px solid ${P.bd}`,borderRadius:4,color:P.tx,fontSize:12,padding:"6px 8px",width:"100%",boxSizing:"border-box",fontFamily:"'DM Sans', sans-serif" }}/></div>
      ...
      {tab==="payroll"&&(<>
        ...
        {["US","PH","IN"].map(ct=>{const pp=d.tm.filter(t=>t.ct===ct&&(t.on||arc));if(!pp.length)return null;const mo=pp.filter(p=>p.on).reduce((s,p)=>s+p.co,0);return(<div key={ct} style={{ marginBottom:20 }}><div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:10 }}><span style={{ fontSize:16 }}>{FL[ct]}</span><span style={{ fontSize:13,fontWeight:700 }}>{ct==="US"?"United States":ct==="PH"?"Philippines":"India"}</span><Bdg c="r">{fmt(-mo)}/mo</Bdg></div><div style={{ display:"grid",gap:6 }}>{pp.map(p=>{const pi=d.tm.indexOf(p);return(<div key={p.id} style={{ background:p.on?P.c1:`${P.c1}80`,borderRadius:8,padding:"10px 14px",border:`1px solid ${P.bd}`,display:"flex",alignItems:"center",gap:12,opacity:p.on?1:.4 }}><div style={{ flex:1 }}><div style={{ display:"flex",alignItems:"center",gap:6 }}><input value={p.nm} onChange={e=>{const nt=[...d.tm];nt[pi]={...p,nm:e.target.value};save({...d,tm:nt});}} style={{ background:"transparent",border:"none",color:P.tx,fontFamily:"'DM Sans', sans-serif",fontSize:12,fontWeight:600,width:110 }}/><span style={{ fontSize:9,padding:"1px 6px",borderRadius:3,background:`${DC[p.dp]||P.td}20`,color:DC[p.dp]||P.td,fontWeight:600 }}>{p.dp}</span></div><input value={p.rl||""} onChange={e=>{const nt=[...d.tm];nt[pi]={...p,rl:e.target.value};save({...d,tm:nt});}} style={{ background:"transparent",border:"none",color:P.tm,fontFamily:"'DM Sans', sans-serif",fontSize:11,marginTop:1 }} placeholder="Role"/></div><select value={p.dp} onChange={e=>{const nt=[...d.tm];nt[pi]={...p,dp:e.target.value};save({...d,tm:nt});}} style={{ background:P.c2,border:`1px solid ${P.bd}`,borderRadius:4,color:P.tx,fontFamily:"'DM Sans', sans-serif",fontSize:11,padding:4 }}>{["Development","Marketing","Operations","Leadership"].map(dd=><option key={dd}>{dd}</option>)}</select><NumIn value={p.co} onChange={e=>{const nt=[...d.tm];nt[pi]={...p,co:+e.target.value};save({...d,tm:nt});}} w={70}/><button onClick={()=>{const nt=[...d.tm];nt[pi]={...p,on:!p.on};save({...d,tm:nt});}} style={{ background:"transparent",border:"none",cursor:"pointer",fontSize:14 }} title={p.on?"Archive":"Reactivate"}>{p.on?"\ud83d\udce6":"\u267b\ufe0f"}</button><button onClick={()=>save({...d,tm:d.tm.filter((_,i)=>i!==pi)})} style={{ background:"transparent",border:"none",color:P.rM,cursor:"pointer",fontSize:13 }}>×</button></div>);})}</div></div>);})}
      </>)}

      {/* ===================== PARTNERSHIPS ===================== */}
      {tab==="partnerships"&&(<>
        {/* V2: Runway impact front and center */}
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20 }}>
          <Card style={{ padding:16,borderLeft:`3px solid ${P.g}` }}>
            <Lbl>Current Runway</Lbl>
            <div style={{ fontSize:36,fontWeight:800,color:mg>=9?P.g:mg>=6?P.a:P.r,fontFamily:"'JetBrains Mono', monospace" }}>{mg} <span style={{ fontSize:14,fontWeight:500,color:P.tm }}>months green</span></div>
          </Card>
          <Card style={{ padding:16,borderLeft:`3px solid ${P.p}` }}>
            <Lbl>With Partnership</Lbl>
            {(()=>{
              const oBl = ov.base.bl.map((b,i) => b + pm.months[i].cum);
              const oMg = oBl.filter(b=>b>0).length;
              return <div style={{ fontSize:36,fontWeight:800,color:oMg>=9?P.g:oMg>=6?P.a:P.r,fontFamily:"'JetBrains Mono', monospace" }}>{oMg} <span style={{ fontSize:14,fontWeight:500,color:P.tm }}>months green</span></div>;
            })()}
          </Card>
        </div>
        {/* V2: Month-by-month delta with real month names */}
        <Card style={{ padding:14,marginBottom:20 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:`repeat(${win.length},1fr)`,gap:3,marginTop:8 }}>
            {win.map((s,i)=>{
              const net = s.inCurrentYear ? pm.months[s.idx].net : 0;
      ...
      {tab==="dev hire"&&(<>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"280px 1fr",gap:20 }}><div><Lbl>Hiring Parameters</Lbl><Sld label="Developers to Hire" value={dh.cnt} onChange={v=>setDh("cnt",v)} min={1} max={5} suf=" devs"/><Sld label="Avg Cost / Dev" value={dh.avg} onChange={v=>setDh("avg",v)} min={300} max={2000} step={50} pre="$" suf="/mo"/><Sld label="Start Month" value={dh.sm} onChange={v=>setDh("sm",v)} min={0} max={11} suf={` (${MO[dh.sm]})`} color={P.p}/><Sld label="Clients Per Dev Capacity" value={dh.cpc} onChange={v=>setDh("cpc",v)} min={0.5} max={4} step={0.5} suf=" clients"/><Sld label="Revenue Per New Client" value={dh.rpc} onChange={v=>setDh("rpc",v)} min={500} max={5000} step={250} pre="$" suf="/mo"/></div><div><div style={{ display:"flex",flexWrap:"wrap",gap:8,marginBottom:16 }}><KPI label="Monthly Cost" value={`$${dm.totalCost.toLocaleString()}`} color={P.r}/><KPI label="Capacity Added" value={`${dm.capacity} clients`} color={P.b}/><KPI label="Revenue @ Full Ramp" value={`$${dm.addedRev.toLocaleString()}/mo`} color={P.g}/><KPI label="Breakeven" value={dm.breakeven>=0?MO[dm.breakeven]:"Not in 2026"} color={dm.breakeven>=0?P.g:P.r}/><KPI label="Cost/Client Capacity" value={`$${Math.round(dm.totalCost/Math.max(dm.capacity,0.1)).toLocaleString()}/mo`} color={P.t}/></div><Lbl>Cumulative Cash Impact (3-month ramp)</Lbl><div style={{ display:"grid",gridTemplateColumns:`repeat(${win.length},1fr)`,gap:2,marginBottom:16 }}>{win.map((s,i)=>{const m=s.inCurrentYear?dm.months[s.idx]:{cum:0};return<div key={i} style={{ height:34,borderRadius:4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,fontFamily:"'JetBrains Mono', monospace",opacity:s.inCurrentYear&&s.idx>=dh.sm?1:.3,background:m.cum>0?P.gB:m.cum>-3000?P.aB:P.rB,color:m.cum>0?P.g:m.cum>-3000?P.a:P.r }}>{s.inCurrentYear?fK(m.cum):"\u2014"}</div>})}</div><div style={{ overflowX:"auto" }}><table style={{ width:"100%",borderCollapse:"collapse",fontSize:11 }}><thead><tr>{["Month","Ramp","Rev","Cost","Net","Cumul."].map(h=><th key={h} style={{ ...th,fontSize:9 }}>{h}</th>)}</tr></thead><tbody>{dm.months.map((m,i)=>(<tr key={i} style={{ opacity:i>=dh.sm?1:.3 }}><td style={{ padding:"4px 8px",textAlign:"right",borderBottom:`1px solid ${P.bd}10` }}>{MO[i]}</td><td style={{ padding:"4px 8px",textAlign:"right",color:P.tm,borderBottom:`1px solid ${P.bd}10`,fontFamily:"'JetBrains Mono', monospace" }}>{m.ramp?`${Math.round(m.ramp*100)}%`:"\u2014"}</td><td style={{ padding:"4px 8px",textAlign:"right",color:P.g,borderBottom:`1px solid ${P.bd}10`,fontFamily:"'JetBrains Mono', monospace" }}>{fmt(m.rev)}</td><td style={{ padding:"4px 8px",textAlign:"right",color:P.r,borderBottom:`1px solid ${P.bd}10`,fontFamily:"'JetBrains Mono', monospace" }}>{m.cost?fmt(-m.cost):"\u2014"}</td><td style={{ padding:"4px 8px",textAlign:"right",fontWeight:600,color:m.net>=0?P.g:P.r,borderBottom:`1px solid ${P.bd}10`,fontFamily:"'JetBrains Mono', monospace" }}>{m.net>=0?"+":""}{fmt(m.net)}</td><td style={{ padding:"4px 8px",textAlign:"right",fontWeight:600,color:m.cum>=0?P.g:P.r,borderBottom:`1px solid ${P.bd}10`,fontFamily:"'JetBrains Mono', monospace" }}>{fmt(m.cum)}</td></tr>))}</tbody></table></div><Card style={{ padding:12,marginTop:16 }}><Lbl>Impact on Unit Economics</Lbl><div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,fontSize:11,marginTop:6 }}><div><span style={{ color:P.td }}>Current devs:</span> <span style={{ fontFamily:"'JetBrains Mono', monospace" }}>{devs.length}</span></div><div><span style={{ color:P.td }}>After hire:</span> <span style={{ color:P.b,fontFamily:"'JetBrains Mono', monospace" }}>{devs.length+dh.cnt}</span></div><div><span style={{ color:P.td }}>Clients/dev:</span> <span style={{ color:P.t,fontFamily:"'JetBrains Mono', monospace" }}>{((aCl+dm.capacity)/(devs.length+dh.cnt)).toFixed(1)}</span></div></div></Card></div></div>
      </>)}
